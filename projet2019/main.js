...
function showCamera(id) {
  if(id == 1) {
    ...
    $('<img></img>', {
      src: 'http://10.34.168.140/mjpg/video.mjpg'
    }).appendTo(container);
  } else {
...
function swapChannel(channel) {
  $.get('http://10.34.168.135/api/tv/channel/' + channel);
}

function powerTv() {
  if(tvOn) {
    $.get('http://10.34.168.135/api/bravia/power/off');
    tvOn = false;
  } else {
    $.get('http://10.34.168.135/api/bravia/power/on');
    tvOn = true;
  }
}
...
function setHueColor(light, that) {
  $.get('http://10.34.168.135/api/lights/color/' + light + '/' + tinycolor($(that).css("background-color")).toName());
}

function powerLights() {
  if(lightsOn) {
    $.get('http://10.34.168.135/api/lights/power/1/off');
    setTimeout(function() { $.get('http://10.34.168.135/api/lights/power/2/off'); }, 500);
    lightsOn = false;
  } else {
    $.get('http://10.34.168.135/api/lights/power/1/on');
    setTimeout(function() { $.get('http://10.34.168.135/api/lights/power/2/on'); }, 500);
    lightsOn = true;
  }
}
